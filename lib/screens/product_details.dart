...
import 'package:byminuser/custom/toast_component.dart';
import 'package:byminuser/screens/product_reviews.dart';
...
class _ProductDetailsState extends State<ProductDetails> {
  ...
  addToCart({mode, context = null, snackbar = null}) async {
    ...
    if (cartAddResponse.result == false) {
      ToastComponent.showDialog(cartAddResponse.message.toString(), context,
          gravity: Toast.center, duration: Toast.lengthLong);
      return;
    } else {
      if (mode == "add_to_cart") {
  ...
  buildQuantityUpButton() => SizedBox(
        width: 36,
        child: IconButton(
            icon: Icon(Icons.hls, size: 16, color: MyTheme.dark_grey),
            onPressed: () {
              if (_quantity < _stock) {
                _quantity++;
                setState(() {});
  ...
  buildQuantityDownButton() => SizedBox(
      width: 36,
      child: IconButton(
          icon: Icon(Icons.menu, size: 16, color: MyTheme.dark_grey),
          onPressed: () {
            if (_quantity > 1) {
              _quantity--;
              setState(() {});
                        ...
                        children: _carouselImageList.map((url) {
                          ...
                          return Flexible(
                            child: GestureDetector(
                              onTap: () async {
                                // print(index);
                                await _imageCarouselController.animateToPage(
                                  index,
