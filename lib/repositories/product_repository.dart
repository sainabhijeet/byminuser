...
class ProductRepository {
  ...
  Future<ProductMiniResponse> getFlashDealProducts(
    ...
    final response = await http.get(Uri.parse("${AppConfig.BASE_URL}/flash-deal-products/" + id.toString()));
    return productMiniResponseFromJson(response.body);
  }
  ...
  Future<ProductMiniResponse> getRelatedProducts({@required int id = 0}) async {
    final response = await http.get(Uri.parse("${AppConfig.BASE_URL}/products/related/" + id.toString()));
    return productMiniResponseFromJson(response.body);
  }

  Future<ProductMiniResponse> getTopFromThisSellerProducts(
      {@required int id = 0}) async {
    final response = await http.get(Uri.parse("${AppConfig.BASE_URL}/products/top-from-seller/" + id.toString()));
    return productMiniResponseFromJson(response.body);
  }
